...
export default function CandidateAssignments({ exams, selectedExam, candidates, assignedCandidates }: { exams: { data: ExamOption[] }; selectedExam?: { data: ExamOption } | null; candidates: { data: Candidate[] }; assignedCandidates: { data: Candidate[] } }) {
    ...
    const { data, setData, post, processing, reset } = useForm<{ exam_id: string; candidate_ids: string[] }>({
        exam_id: exam?.id ?? '',
        candidate_ids: [],
    });
    ...
    return (
        <PortalAppShell title="Assign Candidates">
            <Head title="Assign Candidates" />
            <PageHeader eyebrow="Candidates" title="Assign Candidates to Exam" description="Select an exam, find candidates, and manage exam candidate assignments." actions={<Button asChild type="button" variant="secondary"><Link href="/candidates"><ArrowLeft className="h-4 w-4" />Back</Link></Button>} />
            ...
            <form onSubmit={submit} className="mb-5 rounded-md border border-border bg-white p-4 shadow-sm">
                <div className="grid gap-3 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
                    ...
                    <Button type="submit" disabled={processing || !data.exam_id || selected.length === 0}><UserPlus className="h-4 w-4" />Assign Selected ({selected.length})</Button>
                </div>
            </form>
