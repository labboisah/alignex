...
import { ArrowLeft, Award, BadgeCheck, CreditCard, FileCheck, Save, Trash2 } from 'lucide-react';
            ...
            <section className="mx-auto max-w-7xl">
                ...
                <div className="grid gap-5 xl:grid-cols-[24rem_minmax(0,1fr)]">
                    <div className="space-y-5">
                        ...
                        <CertificatePreview exam={exam} template={activeTemplate} />
                    </div>
                    <div className="space-y-5">
...
function TemplatePanel({ exam, template }: { exam: Exam; template: Template | null }) {
    ...
    const submit = (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        if (template) {
            patch(`${routeBase(exam, 'templates')}/${template.id}`, { preserveScroll: true });
        } else {
            post(routeBase(exam, 'templates'), { preserveScroll: true });
        }
    };
...
function CertificatePreview({ exam, template }: { exam: Exam; template: Template | null }) {
    ...
    return (
        <div className="rounded-md border border-border bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 overflow-hidden rounded-md border p-2" style={{ borderColor: primaryColor, backgroundColor }}>
                <div className="relative min-h-80 border p-6 text-center" style={{ borderColor: accentColor }}>
