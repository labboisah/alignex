...
export default function ShowCenter({ center, can }: Props) {
    const record = center.data;
        ...
        <PortalAppShell title={record.name}>
            ...
            <section className="mx-auto max-w-5xl">
                <PageHeader
                    eyebrow="Center"
                    title={record.name}
                    description="CBT delivery facility profile and operational scope."
                    actions={
                        <>
                            <ProtectedAction allowed={can.update}>
                                <Button asChild type="button" variant="secondary">
                                    <Link href={`/centers/${record.id}/edit`}>
                            ...
                            <ProtectedAction allowed={can.deactivate && record.status !== 'inactive'}>
                                <Button type="button" variant="danger" onClick={() => router.patch(`/centers/${record.id}/deactivate`, {}, { preserveScroll: true })}>
                ...
                <div className="mt-6 rounded-md border border-border bg-white p-5 shadow-sm">
                    <h2 className="text-base font-semibold text-slateDark">Center Details</h2>
