...
export default function QuestionBanks({ professionalSchool, programmes, courses, modules, subjects, questionBanks }: { professionalSchool: any; programmes: any[]; courses: any[]; modules: any[]; subjects: any[]; questionBanks: any[] }) {
    ...
    const isFacilitator = auth.user?.role === 'facilitator';
    ...
    const courseModules = data.course_id ? modules.filter((module) => String(module.course_id) === String(data.course_id)) : modules;
    ...
    const setCourse = (courseId: string) => {
        ...
        setData({
            ...
            module_id: '',
            subject_id: isFacilitator ? '' : data.subject_id,
        });
    };

    const setModule = (moduleId: string) => {
        const module = modules.find((item) => String(item.id) === String(moduleId));
        const course = module ? courses.find((item) => String(item.id) === String(module.course_id)) : null;

        setData({
            ...data,
            module_id: moduleId,
            course_id: module?.course_id ? String(module.course_id) : data.course_id,
            programme_id: course?.programme_id ? String(course.programme_id) : data.programme_id,
            subject_id: isFacilitator ? '' : data.subject_id,
        });
    };
        ...
        <PortalAppShell title="Question Banks">
            ...
            <form onSubmit={submit} className="mb-6">
                <FormSection title="New Question Bank" description="Attach a bank to the programme, course, and module it assesses." footer={<Button disabled={processing}>Save Question Bank</Button>}>
                    <Grid>
                        {!isFacilitator && <Field label="Programme" error={errors.programme_id}><select className={inputClass} value={data.programme_id} onChange={(event) => setData('programme_id', event.target.value)}><option value="">Auto from course</option>{programmes.map((programme) => <option key={programme.id} value={programme.id}>{programme.name}</option>)}</select></Field>}
                        <Field label="Course" error={errors.course_id}><select required className={inputClass} value={data.course_id} onChange={(event) => setCourse(event.target.value)}><option value="">Select course</option>{courses.map((course) => <option key={course.id} value={course.id}>{course.name}</option>)}</select></Field>
                        <Field label="Module" error={errors.module_id}><select required className={inputClass} value={data.module_id} onChange={(event) => setModule(event.target.value)}><option value="">Select module</option>{courseModules.map((module) => <option key={module.id} value={module.id}>{module.name}</option>)}</select></Field>
                        {!isFacilitator && <Field label="Subject" error={errors.subject_id}><select className={inputClass} value={data.subject_id} onChange={(event) => setData('subject_id', event.target.value)}><option value="">Auto from module</option>{subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></Field>}
