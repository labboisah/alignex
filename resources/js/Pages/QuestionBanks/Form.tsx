...
export function QuestionBankForm({ questionBank, subjects, courses = [], statuses, submitLabel }: { questionBank?: QuestionBank; subjects: { data: SubjectOption[] }; courses?: CourseOption[]; statuses: StatusOption[]; submitLabel: string }) {
    const isInstitution = courses.length > 0 || Boolean(questionBank?.institution_id);
    const { data, setData, post, patch, processing, errors } = useForm<QuestionBankFormData>({
        subject_id: questionBank?.subject_id ?? '',
        course_id: questionBank?.course_id ? String(questionBank.course_id) : '',
    ...
    return (
        <form onSubmit={submit}>
            <FormSection
                ...
                description={isInstitution ? 'Create a controlled source for questions under a course.' : 'Create a controlled source for questions under a subject.'}
                footer={
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {isInstitution ? (
                        <Field label="Course" error={errors.course_id}>
                            <select className={inputClass} value={data.course_id} onChange={(event) => setData('course_id', event.target.value)} required>
                                <option value="">Choose course</option>
                                {courses.map((course) => (
                                    <option key={course.id} value={course.id}>
                                        {[course.name, course.code ? `(${course.code})` : '', course.department_name ? `- ${course.department_name}` : ''].filter(Boolean).join(' ')}
                                    </option>
                                ))}
                            </select>
                        </Field>
                    ...
                    <Field label="Code" error={errors.code}>
                        <input className={inputClass} value={data.code} onChange={(event) => setData('code', event.target.value.toUpperCase())} />
                    </Field>
