...
export function SecondarySchoolForm({ school, organizations, statuses }: { school?: School; organizations: Option[]; statuses: Option[] }) {
    ...
    const submit = (event: FormEvent<HTMLFormElement>) => {
        ...
        school ? patch(`/secondary-schools/${school.id}`) : post('/secondary-schools');
    };

    return (
        <form onSubmit={submit}>
            <FormSection
                title="Secondary School Profile"
                description="Register a secondary school as its own academic exam owner."
                footer={<div className="flex justify-end gap-2"><Button asChild variant="secondary"><Link href={school ? `/secondary-schools/${school.id}` : '/secondary-schools'}>Cancel</Link></Button><Button disabled={processing}><Save className="h-4 w-4" />Save</Button></div>}
            >
