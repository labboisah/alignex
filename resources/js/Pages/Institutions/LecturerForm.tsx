...
export function LecturerForm({ courses, lecturer, submitUrl, cancelUrl, mode }: Props) {
    const { data, setData, post, patch, processing, errors } = useForm({
        name: lecturer?.name ?? '',
        email: lecturer?.email ?? '',
        password: '',
        course_ids: (lecturer?.course_ids ?? []).map(String),
    });
    ...
    return (
        <form onSubmit={submit}>
            <FormSection
                title={mode === 'edit' ? 'Edit Lecturer' : 'Register Lecturer'}
                description="Assigned courses control the lecturer's question bank, question, and assessment access."
            >
                ...
                <div className="mt-6">
                    <div className="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-slate-600">Select one or more department courses.</p>
                        </div>
                    ...
                    {courses.length === 0 && (
                        <div className="mt-3 rounded-md border border-dashed border-border bg-slate-50 p-4 text-sm text-slate-600">
                            No courses exist in this department yet.
                        </div>
                    )}
                </div>
                ...
                <div className="mt-6 flex flex-wrap justify-end gap-2">
                    ...
                    <Button disabled={processing || courses.length === 0}>
                        {mode === 'edit' ? 'Save Changes' : 'Create Lecturer'}
                    </Button>
                </div>
            </FormSection>
        </form>
