...
export type FilterCatalog = Record<string, Record<string, any>[]>;
...
export function useRecordFilters<T extends Record<string, any>>(rows: T[], enabled = true, catalog: FilterCatalog = {}) {
    const [search, setSearch] = useState('');
    const [selected, setSelected] = useState<Record<string, string>>({});
    if (!enabled) return { filteredRows: rows, controls: null };
    // Include authorized parents even when they have no child records.
    const sources = Object.fromEntries(fields.map(([key]) => [key, [
        ...rows,
        ...(catalog[key] ?? []).map(option => ({...option, [key + '_id']: option.id, [key + '_name']: option.name})),
    ]]));
    ...
    const controls = <section aria-label="Filter records" className="mb-4 space-y-3 rounded border border-border bg-white p-4">
        <div className="flex flex-wrap items-end gap-3">
            ...
            {fields.map(([key, label], index) => {
                const previousKeys = fields.slice(0, index).map(([k]) => k);
                const source = sources[key];
                const options = Array.from(new Map(source.filter(row =>
                    ['status', 'difficulty'].includes(key) || previousKeys.every(parent =>
                        !selected[parent] || !Object.prototype.hasOwnProperty.call(row, parent + '_id') ||
                        entry(row, parent).value === selected[parent])
                ).map(row => {
                    const option = entry(row, key);
                    return [option.value, option] as const;
                })).values()).filter(option => option.value).sort((a,b) => a.label.localeCompare(b.label));
                if (!source.some(row => entry(row, key).value)) return null;
