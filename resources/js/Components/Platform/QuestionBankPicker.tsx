...
export default function QuestionBankPicker({ banks, courseMode, value, onChange, disabled = false }: {banks:ImportBank[];courseMode:boolean;value:string;onChange:(bank:ImportBank|null)=>void;disabled?:boolean}) {
    ...
    const parents = Array.from(new Map(banks.map(bank => [parentValue(bank), {value:parentValue(bank),label:bank[labelKey] ?? (bank[key] ? String(bank[key]) : courseMode ? 'Without course' : 'Without subject')}])).values());
    const parentBanks = parent ? banks.filter(bank => parentValue(bank) === parent) : [];
    const modules = Array.from(new Map(parentBanks.filter(bank=>bank.module_id).map(bank=>[String(bank.module_id),{value:String(bank.module_id),label:bank.module_name ?? String(bank.module_id)}])).values());
    ...
    return <>
        ...
        {parent && available.length===0 && <p role="status" className="text-sm text-slate-600">No question banks match this selection.</p>}
    </>;
}
