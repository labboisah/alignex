...
class CandidateClientDownloadController extends Controller
{
    public function __invoke(Request $request): BinaryFileResponse|RedirectResponse
    {
        ...
        $downloadUrl = trim((string) config('alignex.apps.candidate_app_download_url'));
        ...
        if ($release && AppArtifactFile::isDownloadable($release->absolutePath(), AppRelease::ARTIFACT_CLIENT_APP)) {
